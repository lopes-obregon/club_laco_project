...
namespace CompetFácil.View
{
    public partial class ViewEquipeForm : Form
    {
        ...
        public ViewEquipeForm(object? equipeSelecionado)
        {
            InitializeComponent();
            InitialList();
            InitToAtualizar(equipeSelecionado);
        }

        private void InitToAtualizar(object? equipeSelecionadoUi)
        {
            EquipeController.LoadEquipe(equipeSelecionadoUi);
            var equipe = EquipeController.equipe;
            if (equipe != null)
            {

                nomeEquipeTextBox.Text = equipe.NomeEquipe;
                membroslistBox.Items.AddRange(equipe.Laçadores.ToArray());
            }
        }
